...
- **音效与形象各自独立**：3 种音效（大狗叫 / 叮咚鸡 / 哈基米）只决定声音采样，4 种内置形象（大狗 / 叮咚鸡 / 哈基米 / 东海帝皇）只决定画面角色，互不关联。顶部「音乐」右侧有两个并排的内联按钮——「音效」循环切声音（含「关闭音效」一档）、「形象」循环切画面角色（含东海帝皇，透明帧动画，首次选择后按需加载图集）。形象选择会保存在 `localStorage`，刷新不丢。
- **自定义形象**：点「形象」按钮旁的「+」打开面板，上传「闭嘴」「张嘴」两张图即可生成自己的形象（图片经 canvas 压缩成 PNG dataURL 存 `localStorage`，刷新仍在，可随时删除）。点「形象」按钮即可在内置形象与所有自定义形象间循环切换。制作图片请参考根目录 `提示词.txt`，详见下方「自定义形象」一节。
- **两种演奏模式**：
...
内置 4 种形象之外，可上传自己的图做成新形象：

1. 点击顶部「形象」按钮右侧的「**+**」，打开「添加自定义形象」面板。
2. 点「闭嘴」槽选一张图、点「张嘴」槽选一张图（两个槽可任意顺序选；保存时会自动把张嘴图对齐到闭嘴图尺寸，保证张嘴动画不跳）。
3. 点中间预览区可切换查看张嘴效果，确认无误后填名称（≤6 字），点「保存并使用」。
4. 新形象立即生效，并保存在浏览器 `localStorage`（刷新、重开浏览器都不丢）。之后点「形象」按钮即可在内置形象与所有自定义形象间循环。
5. 再次打开「+」面板，下方「已添加的自定义形象」里点 × 可删除。

> 制作图片要求：两张图**尺寸完全相同、画面严格对齐**（除嘴巴外一个像素都不能动），建议透明背景 PNG。最稳的做法是先画好闭嘴图，再在其基础上只改嘴巴。完整规格、可直接复制的 AI 绘图 Prompt 与验收清单见根目录 **`提示词.txt`**。

注意：自定义图以 PNG dataURL 存 `localStorage`（约 5MB 上限），图片过大会导致保存失败；网页已自动压缩到 360px 宽，但请勿上传超大原图。
...
- **本地存储**：演奏设置通过一个本地适配器（`localToyAdapter`）以 `localStorage` 持久化，键名沿用历史命名；形象选择（`dagou-selected-character-v1`）与自定义形象（`dagou-custom-characters-v1`，闭嘴/张嘴图压缩为 PNG dataURL）也单独存 `localStorage`，刷新与重开浏览器都不丢。
