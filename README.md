...
- **数字键 1–8 速弹**（两种模式通用，含小键盘 Numpad1–8）：钢琴模式对应 8 个白键（低→高），普通模式 1–4 与 5–8 重复覆盖 4 个分区；音节不用修饰键——同一数字键每按一次轮换 大→狗→叫→大…（各键独立计数，页面刷新重置）。按住延音、松开停止，节拍吸附与特效都和点击完全一致。
- 全屏几何特效（12 种）、节拍律动、张嘴动画、按键网格显示。
- **版本标识**：大狗 Tap 页面右下角低调显示当前版本号 `v1.0.260819-ui4`（半透明灰、`pointer-events:none` 不挡互动；改版本直接编辑 `public/dagou-tap.html` 里 `.version-tag` 的文本）。
