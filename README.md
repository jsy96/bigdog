...
- **版本标识**：界面右下角低调显示当前版本号 `v1.0.260811-ios2`（半透明灰、`pointer-events:none` 不挡互动；改版本直接编辑 `public/index.html` 里 `.version-tag` 的文本）。
...
scripts/
  ...
  build-audio-data-json.py  把 public/audio-data.js（源 WAV）转换成 audio-data.json（m4a/AAC-LC，前端 fetch 异步加载；内置 YIN 音准校验）
...
首次进入需点击一次以解锁音频（浏览器自动播放策略）；iPhone / iPad 若处于静音模式或媒体音量过低，仍需先打开铃声/媒体音量，之后即可演奏。
...
- **iOS 音频启动**：`audio-data.json` 仍使用 iOS 友好的 m4a/AAC 包；`AudioContext` 只在首次手势内创建，并在同一个同步手势调用段内立即 `resume()` + 启动 1 帧静音 `AudioBufferSourceNode`，解决部分 iOS Safari / 内嵌 WebView 只 `resume()` 仍静音的问题；`resume()` 超时不再视为加载失败，而是保留当前音频上下文，后续 `pointerdown` / `touchstart` / `click` 会继续尝试 `resume()` 并再次暖机。iOS WebKit 上音效样本改为串行 `decodeAudioData`，每个解码带超时兜底；若 WebKit 偶发不回调，会显示可重试错误并清理旧 `AudioContext`，不会永久停在「狗叫加载中」。
...
# 把 audio-data.js（源 WAV）转成 audio-data.json（m4a/AAC-LC），前端 fetch 异步加载（重建音频后重跑：先 build_audio_data.mjs 再本脚本）
uv run --no-project --with numpy scripts/build-audio-data-json.py
```

音高分析依赖 Python 3 + NumPy（本机用 uv 管理），角色动画压缩依赖 Node.js + FFmpeg，音频 WAV→m4a/AAC 转换依赖 ffmpeg + NumPy（脚本内置 YIN 音准校验）。所有报告与临时产物放在 `tools/tmp/`，生产页面不得依赖其中任何文件。
