...
- **顶部按钮（明显 + 可读 + 不溢出）**：6 个按钮全部「图标 + 文字标签」（音乐 / ha-ji-mi / 形象 / 添加 / 钢琴 / C4），底色实心暖白 + 深灰描边 + 微渐变高光，对比度大幅提高；min-height 48px（窄屏 50px）、图标 24px、字号 14px。窄屏（≤720px）按钮组 `flex-wrap` 自动换行成 3+3 两行，每行 3 个平分宽度，文字标签在所有尺寸都保留（不再收成纯图标），手机上不再溢出或看不懂。
