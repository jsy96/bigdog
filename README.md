...
> 作者：马克杯 MarkCup（bilibili 创作者）

纯前端单页应用，**无后端依赖、无第三方 SDK**。点按或拖动屏幕任意位置即可触发音效——三套音色（大狗叫 / 哈基米 / 叮咚鸡）的每个分区都被对准 A 小调五声音阶的固定音高，配合 128 BPM 的 C–G–Am–F 背景循环，自由演奏也不会刺耳。

> 本项目原为 bilibili Toy 互动玩具，依赖 b站 Toy SDK 做云端同步与看视频解锁。
> 现已**移除全部 Toy SDK 依赖**，改为独立纯前端网页：所有音色 / 形象直接解锁，演奏设置用浏览器 `localStorage` 本地持久化，作者主页与开发视频以新标签打开 bilibili。
...
- **哈基米形象**：原皮 / 帝皇（东海帝皇透明帧动画，首次选择后按需加载图集）。
- **两种演奏模式**：
  - 普通模式：4 个固定音高分区（横屏左→右，竖屏上→下）。
  - 钢琴模式：一个八度白键，可在屏幕边缘切换 **C3–C7 四档起始八度**。
- **长按延音原位换调**：`jiao / mi` 长音循环中滑动只切 `playbackRate`，不重新播放开头。
- **演奏设置本地持久化**：钢琴模式、八度切换、强化节奏、显示网格、起始八度均保存在 `localStorage`。
- **键盘控制**：钢琴模式下用键盘行（QWE…/ASD…/ZXC…）弹奏，方向键 ←/→ 切换八度。
- 全屏几何特效（12 种）、节拍律动、张嘴动画、可选按键网格显示。
- 作者主页 / 开发视频以新标签打开 bilibili。
...
Image/                  图片资源（角色闭嘴/张嘴图、帝皇图集、头像、视频封面）
docs/
...
- **本地存储**：移除 Toy SDK 后，演奏设置通过一个本地适配器（`localToyAdapter`）以 `localStorage` 持久化，键名沿用历史命名。
